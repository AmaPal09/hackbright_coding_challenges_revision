...
def long_words(words):
    ...
    for word in words: 
        if len(word) > 4: 
            word_greater_than_4.append(word)

    return word_greater_than_4
...
def n_long_words(words, n):
    ...
    for word in words: 
        if len(word) > n: 
            word_greater_than_n.append(word)
    return word_greater_than_n
...
def average(numbers):
    ...
    """
    total_list = sum_numbers(numbers)
    try: 
        average_list = total_list/len(numbers)
    except ZeroDivisionError: 
        print('Average cannot be calculated for an empty list')
...
def duplicates(items):
    ...
    for item1 in items: 
        ...
        for item2 in items[item1_index:len(items)]: 
            print(item1, item2, items[item1_index+1:len(items)])
            if item1 == item2: 
                list_of_dups.append(item2)
